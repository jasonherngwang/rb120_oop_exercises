...
employee must have a unique serial number. However, when we are testing 
our program, an exception is raised. Fix the code so that the program works 
as expected without error.

The getter method Employee#serial_number is private, which means that it 
can only be invoked by the current instance of Employee. It cannot be invoked 
with an explicit caller from outside the instance.

The EmployeeManagementSystem#exists? method invokes the Employee#== method 
which invokes the getter #serial_number on another instance of Employee.
To fix this, move the #getter serial_number to the protected section of 
the class.
=end
